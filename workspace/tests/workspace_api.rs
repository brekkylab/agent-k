...
//! testable standalone through `workspace::…` alone.

use bytes::Bytes;
use futures_util::StreamExt;
use uuid::Uuid;
use workspace::{OpenOptions, ReadDirMeta, WorkspaceFs};
...
async fn local_file_round_trip_through_public_api() {
    ...
    let st = fs.metadata("/note.txt").await.unwrap();
    assert!(st.is_file());
    assert_eq!(st.len, 15);
    ...
    // read_dir lists it.
    assert!(dir_names(&fs, "/").await.contains(&"note.txt".to_string()));
}
...
async fn create_and_remove_dir_through_public_api() {
    ...
    assert!(dir_names(&fs, "/").await.contains(&"sub".to_string()));
