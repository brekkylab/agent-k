...
-- every row. Unlike the client pair that `0003` removed it is not a secret, but it carries
...
--
-- All three keys go together so a row's content converges whether or not `0003` already
...
-- earlier UPDATE already freed -- measured, 20 rows on WAL: after `0003` and this one, 9 of
...
--
-- So the remedy is rotation, not deletion: on any database that ran the pre-0003 code the
