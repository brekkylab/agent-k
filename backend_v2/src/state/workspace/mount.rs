...
impl WorkspacesState {
    ...
    pub async fn create_mount(&self, mut mount: WorkspaceMount) -> StateResult<WorkspaceMount> {
        ...
        // Mounts changed → rebuild the Vfs on next access.
        self.invalidate_vfs(mount.workspace_id);
        Ok(mount)
    }
    ...
    pub async fn remove_mount(&self, id: Uuid) -> StateResult<WorkspaceMount> {
        ...
        sqlx::query("DELETE FROM workspace_mounts WHERE id = ?")
            ...
            .execute(&self.db)
            .await?;
        self.invalidate_vfs(existing.workspace_id);
        Ok(existing)
    }
...
mod tests {
    ...
    /// A mount create/remove must evict the cached `Vfs` so the next `get_fs`
    /// reflects the change. Checked via the root mount-name list (no network).
    #[tokio::test]
    async fn get_fs_reflects_mount_changes() {
        ...
        let root = ForwardFs::readdir(&fs, "/").await.unwrap();
        assert!(
            root.iter().any(|e| e.name == "s3-prod"),
