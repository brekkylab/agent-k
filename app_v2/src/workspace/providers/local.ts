...
export const localProvider: SourceProvider = {
  id: 'local',
  nameKey: 'workspace.src.local',
  category: 'files',
  kind: 'files',
  connected: true,
  attachable: true,
  ...
  async list(ctx: ListCtx): Promise<SourceEntry[]> {
    const stats = await listDirectory(ctx.path ?? '/');
    return stats.map(statToEntry);
  },
  async recent(): Promise<SourceEntry[]> {
    const stats = await listDirectory('/');
    return stats
  ...
  async detail(id: string): Promise<SourceDetail> {
    // The id is the normalized webdav path — list its parent directory so
    // nested entries (e.g. "/reports/q2.pdf") resolve, not just root files.
    ...
    if (!stat) throw new Error(`local: no entry for ${id}`);
