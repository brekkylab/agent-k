...
export function makeNotionProvider(mount: MountResponse): SourceProvider {
  const prefix = barePrefix(mount.prefix);
  ...
  return {
    ...
    recent: async () =>
      (await listAll())
        ...
        .slice(0, 20),
    detail: async (pageId: string): Promise<SourceDetail> => {
      ...
      const text = await getFileText(`/${prefix}/pages/x__${pageId}/page.json`);
